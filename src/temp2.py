    ...
    if args[i]["agent"] == "BayesianFiniteVariantsAgent":
        ...
        if args[i]["simulate_type"] == "markov_chain":
            ...
            if PLOT_OBJS == "distance":
                plt_data.append(rec.distance, axis=0)
            elif PLOT_OBJS == "expected_distance":
                if rec.expected_distance is None:
                    rec.compute_expected_distance()
                plt_data.append(np.sum(rec.expected_distance, axis=0))
            elif PLOT_OBJS == "oldness":
                plt_data.append(rec.oldness, axis=0)
            elif PLOT_OBJS == "expected_oldness":
                if rec.expected_oldness is None:
                    rec.compute_expected_oldness()
                plt_data.append(rec.expected_oldness)
            else:
                plt_data.append(rec[list(rec.keys())[0]])

        elif args[i]["simulate_type"] == "monte_carlo":
            if PLOT_OBJS == "distance":
                plt_data.append(rec.distance)
            elif PLOT_OBJS == "expected_distance":
                plt_data.append(rec.expected_distance)
            elif PLOT_OBJS == "oldness":
                plt_data.append(rec.oldness)
            elif PLOT_OBJS == "expected_oldness":
                plt_data.append(rec.expected_oldness)
            else:
                plt_data.append(rec[list(rec.keys())[0]])
...
def comulative_average(data):
    ...
    print(cumsum.shape, counts.shape)
    ...
    for i in range(setting_count):
        for ai in range(plt_data[i].shape[1]):
            for aj in range(ai,plt_data[i].shape[2]):
                ax[i].plot(np.log(comulative_average(plt_data[i][:, ai, aj])))
        ax[i].get_xaxis().set_visible(False)
        ax[i].get_yaxis().set_visible(False)
    plt.savefig(DATA_DIR + '/fig/'+ "comulative_average_log.png")
    plt.show()
# fig_num = 10
