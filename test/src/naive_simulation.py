...
# --- 再開用: 既存ファイルの確認 ---
state_files = sorted(glob.glob(os.path.join(save_dir, "state_*.npy")),
                    key=lambda x: int(os.path.splitext(os.path.basename(x))[0].split('_')[1]))
distance_files = sorted(glob.glob(os.path.join(save_dir, "distance_*.npy")),
                       key=lambda x: int(os.path.splitext(os.path.basename(x))[0].split('_')[1]))
random_state_files = sorted(glob.glob(os.path.join(save_dir, "random_state_*.pkl")),
        ...
        with open(csv_path, "r", newline="") as csvfile:
            ...
            for row in reader:
                if int(row["save_idx"]) == last_idx:
                    t = int(row["t"])
            if t is None:
                raise ValueError(f"save_idx_t_map.csvにsave_idx={last_idx}の記録がありません")
    else:
        print(f"save_idx_t_map.csvが見つかりません。おおよそのtを計算します。それでもよければ y を入力してください。")
        if input().strip().lower() != 'y':
            raise FileNotFoundError(f"save_idx_t_map.csvが見つかりません。save_dir={save_dir}を確認してください。")
        t = (last_idx + 1) * save_interval_max
